...
describe('task-completed-handler.cjs', () => {
  ...
  describe('Evidence advisory check', () => {

    function writeEvidence(planDir, { sealed }) {
      const ev = path.join(planDir, 'evidence');
      fs.mkdirSync(ev, { recursive: true });
      fs.writeFileSync(path.join(ev, 'study-context.json'), JSON.stringify({
        task: 't', mode: 'auto', acceptanceCriteria: ['x'], touchpoints: ['a'], blastRadius: ['b'], contracts: ['c'],
      }));
      fs.writeFileSync(path.join(ev, 'temper-results.json'), JSON.stringify({
        commands: [{ command: 'node --test', exitCode: sealed ? 0 : 1, status: sealed ? 'pass' : 'fail', summary: sealed ? 'green' : 'red', ts: '2026-06-16T00:00:00.000Z' }],
      }));
      fs.writeFileSync(path.join(ev, 'inspection-verdict.json'), JSON.stringify({
        score: 9, criticalCount: 0, decision: 'SEALED', acceptanceCovered: ['x'], regressionChecked: ['b'],
        contractStatus: 'OK', refuted: [], unproven: [], reachableRegressions: [],
      }));
    }

    it('surfaces evidence gaps (advisory) for a plan-backed solo task and still exits 0', async () => {
      const tmpDir = path.join(os.tmpdir(), 'tc-hook-ev-gap-' + Date.now());
      const planDir = path.join(tmpDir, 'plan');
      fs.mkdirSync(planDir, { recursive: true });
      try {
        writeEvidence(planDir, { sealed: false });
        const result = await runHook({ task_id: '1', task_subject: 'Plan task', plan_dir: planDir });
        assert.strictEqual(result.exitCode, 0);
        assert.ok(result.output, 'should emit advisory context even without a team');
        assert.match(result.output.hookSpecificOutput.additionalContext, /Evidence gaps|advisory/i);
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    });

    it('never blocks: sealed evidence on a solo task exits 0 with a clean note', async () => {
      const tmpDir = path.join(os.tmpdir(), 'tc-hook-ev-ok-' + Date.now());
      const planDir = path.join(tmpDir, 'plan');
      fs.mkdirSync(planDir, { recursive: true });
      try {
        writeEvidence(planDir, { sealed: true });
        const result = await runHook({ task_id: '2', task_subject: 'Sealed task', plan_dir: planDir });
        assert.strictEqual(result.exitCode, 0);
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    ...
    it('ignores tasks whose plan has no evidence dir', async () => {
      const tmpDir = path.join(os.tmpdir(), 'tc-hook-ev-none-' + Date.now());
      const planDir = path.join(tmpDir, 'plan');
      fs.mkdirSync(planDir, { recursive: true });
      try {
        const result = await runHook({ task_id: '3', task_subject: 'No evidence', plan_dir: planDir });
        assert.strictEqual(result.exitCode, 0);
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
