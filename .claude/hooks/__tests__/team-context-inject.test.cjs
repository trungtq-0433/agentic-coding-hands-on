 ...
 * - TKM stack context building from environment variables
...
describe('team-context-inject.cjs', () => {
  ...
  describe('TKM Stack Context Building', () => {

    it('includes TKM context when environment variables are set', async () => {
      ...
      try {
        ...
        const result = await runHook({
          ...
          env: {
            HOME: tmpDir,
            TKM_REPORTS_PATH: '/project/plans/reports',
            TKM_PLANS_PATH: '/project/plans',
            TKM_PROJECT_ROOT: '/project',
            TKM_GIT_BRANCH: 'main'
          }
        });
        ...
        assert.ok(context.includes('TKM Context'), 'Should have TKM Context section');
    ...
    it('always includes commit convention in TKM context', async () => {
    ...
    it('only includes TKM context lines when env vars exist (fail-open)', async () => {
      ...
      try {
        ...
        // Should not have empty TKM Context section
        // Commit convention is always present, so TKM Context should appear
  ...
  describe('Error Handling & Robustness', () => {
    ...
    it('logs to stderr only when TKM_DEBUG is enabled', async () => {
