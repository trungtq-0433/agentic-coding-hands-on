#!/usr/bin/env node
/**
 * Tests for session temp-state locking in tkm-config-utils.cjs
...
const LIB_PATH = path.join(__dirname, '..', 'lib', 'tkm-config-utils.cjs');
...
describe('tkm-config-utils session state locking', () => {
