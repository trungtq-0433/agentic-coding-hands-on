#!/usr/bin/env node
/**
 * Usage-limits cache — read, write, and fetch helpers for quota display.
 *
 * Owns two on-disk caches under $TMPDIR (overridable via env):
 *   sk-usage-limits-cache.json           — raw API data + snapshot, TTL 60 s
 *   sk-usage-limits-cache.json.eligibility — subscription eligibility, TTL 60 s
 *
 * fetchUsageLimits() calls https://api.anthropic.com/api/oauth/usage with the
 * Claude OAuth access token. Writes are atomic (write-to-tmp, rename).
 * Called by the statusline hook; no event firing here.
 * @module usage-limits-cache
 */
'use strict';
