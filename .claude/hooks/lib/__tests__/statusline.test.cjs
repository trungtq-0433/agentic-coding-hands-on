...
test('getGitInfo timeout does not cache null result', () => {
  ...
  const originalTimeout = process.env.TKM_GIT_TIMEOUT_MS;
  ...
  try {
    ...
    process.env.TKM_GIT_TIMEOUT_MS = '50';
    ...
    if (originalTimeout == null) delete process.env.TKM_GIT_TIMEOUT_MS;
    else process.env.TKM_GIT_TIMEOUT_MS = originalTimeout;
