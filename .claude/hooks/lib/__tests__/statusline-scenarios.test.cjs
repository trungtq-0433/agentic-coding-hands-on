...
function runStatuslineSync({ payload, cwd = TEST_ROOT, env = {}, inputRaw = null, timeoutMs = 20000 }) {
  ...
  const eligibilityCachePath = env.TKM_USAGE_ELIGIBILITY_CACHE_PATH
    || path.join(os.tmpdir(), `sk-usage-eligibility-statusline-test-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.json`);
  const runtimeEnv = {
    ...process.env,
    TKM_USAGE_CACHE_PATH: USAGE_CACHE_PATH,
    TKM_USAGE_ELIGIBILITY_CACHE_PATH: eligibilityCachePath,
    ...env
  };
  if (!Object.prototype.hasOwnProperty.call(env, 'NO_COLOR')) delete runtimeEnv.NO_COLOR;
  if (!Object.prototype.hasOwnProperty.call(env, 'HOME') && fs.existsSync(path.join(cwd, '.claude', '.tkm.json'))) {
    runtimeEnv.HOME = cwd;
  }
...
function withQuotaEligibilityCache(payload, fn) {
  ...
  try {
    ...
    return fn({ TKM_USAGE_ELIGIBILITY_CACHE_PATH: cachePath });
  } finally {
...
async function main() {
  ...
  await test('Optional stdin timeout forces fallback when enabled', async () => {
    const result = await runStatuslineWithDelayedChunks({
      ...
      delaysMs: [400, 0],
      env: { TKM_STATUSLINE_STDIN_TIMEOUT_MS: '100' }
    });
