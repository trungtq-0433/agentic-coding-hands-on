...
describe('context-builder.cjs', () => {
  ...
  describe('Section builders', () => {
    ...
    it('buildPlanContextSection injects SDD mode line when sddMode provided', () => {
      const lines = contextBuilder.buildPlanContextSection({
        planLine: '- Plan: none',
        reportsPath: 'plans/reports/',
        validationMode: 'prompt',
        validationMin: 3,
        validationMax: 8,
        sddMode: 'on'
      });
      assert.ok(lines.some(l => l.includes('SDD mode: on')), 'Should include SDD mode line');
      assert.ok(lines.some(l => l.includes('takumi.sddMode')), 'Should reference the config key');
    });

    it('buildPlanContextSection flags first-run prompt when sddMode is ask', () => {
      const lines = contextBuilder.buildPlanContextSection({
        planLine: '- Plan: none',
        reportsPath: 'plans/reports/',
        validationMode: 'prompt',
      ...
      const sddLine = lines.find(l => l.includes('SDD mode'));
      assert.ok(sddLine && sddLine.includes('prompt on first run'), 'ask state should hint at first-run prompt');
    });

    it('buildPlanContextSection omits SDD line when sddMode absent', () => {
      const lines = contextBuilder.buildPlanContextSection({
        planLine: '- Plan: none',
        reportsPath: 'plans/reports/',
        validationMode: 'prompt',
      ...
      assert.ok(!lines.some(l => l.includes('SDD mode')), 'Should not inject SDD line without sddMode');
    });
