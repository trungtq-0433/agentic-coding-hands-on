...
/**
 * Statusline activity section renderers — agent flow and todo status lines.
 * Produces 0-2 output lines each; lives outside the flat section registry
 * because multi-line sections need their own composition pass.
 *
 * @module statusline-activity-renderers
 */
...
/**
 * Render agent flow as a compact chronological chain with duplicate collapsing.
 * Line 1: ○ type ×N → ● type (N done)
 * Line 2: ▸ description (elapsed)
 *
...
function renderAgentsLines(transcript, maxRows, sectionConfig = {}) {
  ...
  // Merge consecutive same-type/status runs before slicing — keeps the flow line terse
  const collapsed = [];
  ...
  // maxRows=0 hides agents entirely
  ...
  // Detail line: prefer running agent; fall back to last completed
...
/**
 * Render todo status as a single line: in-progress task, next pending, or all-done summary.
 *
 * @param {Object} transcript - Activity snapshot { agents, todos }
