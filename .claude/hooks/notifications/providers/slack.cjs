...
module.exports = {
   ...
   * SLASK_WEBHOOK_URL is read as fallback: a past rebrand sweep corrupted
   * SLACK -> SLASK, so some users configured the misspelled name to make it work.
   * @param {Object} env - Environment variables
   * @returns {boolean} True if SLACK_WEBHOOK_URL (or legacy SLASK_WEBHOOK_URL) is set
   */
  isEnabled: (env) => !!(env.SLACK_WEBHOOK_URL || env.SLASK_WEBHOOK_URL),
  ...
  send: async (input, env) => {
    ...
    return send('slack', env.SLACK_WEBHOOK_URL || env.SLASK_WEBHOOK_URL, payload);
  }
};
