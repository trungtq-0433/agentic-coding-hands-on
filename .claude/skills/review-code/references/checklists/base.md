...
The baseline checklist, good for any kind of project. It runs in two passes: critical (blocking) and informational (non-blocking).

## Instructions

Take `git diff origin/main` and look for the problems listed below. Be concrete — name the `file:line` and propose a fix. Leave the clean parts alone. Flag only what's genuinely wrong.
...
Nothing to report? `Pre-Landing Review: No issues found.`

Keep it tight: one line for the problem, one for the fix. No preamble.
