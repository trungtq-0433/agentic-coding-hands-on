...
Stacks on top of `base.md`. Pull it in whenever a frontend framework is in play (React, Vue, Svelte, Next.js, and the like).

## Detection

Layer this overlay on when any of the following holds:
- `package.json` lists a `react`, `vue`, `svelte`, `next`, `nuxt`, or `angular` dependency
- Directories like `src/pages/`, `src/app/`, `src/components/`, `src/views/` exist
- The diff touches HTML/JSX/TSX/Vue files
