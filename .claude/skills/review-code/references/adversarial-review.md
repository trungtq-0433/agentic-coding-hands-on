...
Graph context, if `graphify-out/graph.json` exists (Knowledge Graph is on by default):
- Load `../../_shared/graphify-code-graph.md`.
- Include only graph paths/affected nodes that explain ADDED/MODIFIED code impact.
- Treat graph output as context, not a finding by itself.
...
**Evidence contract:** when this review feeds an `inspection-verdict.json` (see `_shared/references/evidence-artifacts.md`), every adjudicated finding here becomes one `findings[]` entry -- `severity`/`category`/`summary` map straight across, and the verdict here (Accept/Reject/Defer) becomes `disposition`. The `LOCATION: file:line` reported below is not optional prose for an Accept finding -- it becomes the entry's `location`, and the evidence gate machine-checks it (`path:NNN` or `path:NN-MM`). An Accept finding with no real location blocks a hard stage. Reject/Defer findings may carry `location` for traceability but it is not enforced.
