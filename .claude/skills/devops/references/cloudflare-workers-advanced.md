...
Heavier techniques for squeezing out performance and wiring up the trickier workflows.
...
### Bundle Size
- Hold the bundle under <1MB bundled
- Drop dependencies you don't use
- Split code so heavy paths load on demand
- Check with: `wrangler deploy --dry-run --outdir=dist`

### Cold Starts
- Keep startup code to a minimum
- Prefer bindings over fetch
- Don't pull big imports at the top level

### Memory Management
- Close pages once you're done: `await page.close()`
- Let go of browsers: `await browser.disconnect()`
- Set cleanup alarms in Durable Objects

### Request Optimization
- Filter on the server with `--filter`
- Group writes with D1 `.batch()`
...
- [ ] Error handling covers every stage
- [ ] Rate limiting configured
- [ ] Caching strategy decided and wired in
- [ ] Secrets managed with `wrangler secret`
- [ ] Health checks in place
- [ ] Monitoring alerts wired up
- [ ] Browser rendering reuses sessions
- [ ] Resources cleaned up (pages, browsers)
- [ ] Timeouts set sensibly
