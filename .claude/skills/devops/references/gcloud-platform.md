...
Working reference for gcloud, the command-line front end to Google Cloud Platform.
...
Why impersonation? You get short-lived tokens instead of long-lived key files, and access stays managed in one place.
...
- Prefer service account impersonation over key files
- Grant the least IAM access a task needs
- Rotate keys on a schedule

### Performance
- Push the filtering server-side: `--filter`
- Trim the result set: `--limit=10`
- Pull back only the fields you use: `--format="value(name)"`
- Fire batched work off with `--async`

### Maintainability
- Keep one named configuration per environment
- Document the commands you run
- Drive values from environment variables
