...
The mental model and day-to-day moves for shipping apps as Docker containers.

## Core Concepts

**Containers:** An app plus its dependencies running as an isolated, low-overhead process. Treat them as disposable — they leave nothing behind unless you tell them to.

**Images:** The immutable template a container boots from. Built in layers so common pieces get cached and shared.

**Volumes:** Storage that lives independently of the container, so data outlasts the container that wrote it.

**Networks:** The wiring that lets containers talk to each other.
...
Benefits: leaner final image, a smaller attack surface, and no compilers or build tooling riding along into production.
...
- Pin an exact image version — never trust `latest` to stay put
- Drop privileges: run as a non-root user
- Reach for multi-stage builds to keep the final image small
- Wire up health checks
- Cap CPU and memory with resource limits
- Aim to keep images under 500MB
- Scan for known CVEs: `docker scout cves myapp:1.0`
