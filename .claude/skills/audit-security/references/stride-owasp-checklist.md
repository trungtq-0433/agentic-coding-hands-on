...
The working reference behind a `tkm:audit-security` pass — lean on it while running Step 2 (STRIDE Analysis) and Step 3 (OWASP Top 10 Check).
...
Run these regexes across the source. Treat every hit as a Critical finding until proven otherwise.
...
> Cut the noise: ignore hits in `*.test.*`, `*.spec.*`, `*.example`, and `*.md` files where the value is plainly a stand-in (e.g., `YOUR_KEY_HERE`, `<your-token>`).

---

## Dependency Audit Commands

Match the command to the stack in front of you, run it, and fold its output into the findings report:
