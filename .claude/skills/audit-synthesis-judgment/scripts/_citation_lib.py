"""Citation parsing and resolution helpers.

COPIED VERBATIM from `audit-doc-parity/scripts/_citation_lib.py` so this skill is self-contained
(audit-doc-parity may not be installed; rebuild-spec — whose output this audits — always is, and the
copy still resolves rebuild-spec's `_lang_lib` as a sibling). `tests/test_citation_lib_parity.py`
asserts the citation regex + traversal guard still match parity's; keep the two in sync when parity's
citation/path-guard logic changes.
