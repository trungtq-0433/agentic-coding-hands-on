...
A grab-bag of verify commands sorted by domain — lift one straight into your tkm:auto-research config.
Direction reads as: **lower** when fewer errors/ms/bytes wins; **higher** when more coverage/accuracy wins.
...
| Rule | Detail |
|------|--------|
| One number | the final stdout line is a bare number, integer or float — nothing else |
| Exit codes | exit 0 means the reading counts; anything else is a crash, logged and skipped |
| Runtime | stay under 30s; sample instead of measuring everything on heavy workloads |
| Determinism | if it wobbles run to run, mark `noise: high` and take 3–5 runs |
| Units | hold the unit steady across every round — never swap it mid-loop |
| Direction | spell it out: is `lower` or `higher` the win? |
