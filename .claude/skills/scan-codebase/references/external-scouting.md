...
Before using Gemini/OpenCode, check for `graphify-out/graph.json` (Knowledge Graph is on by default). If present, load
`../../_shared/graphify-code-graph.md` from the skills root and run graph-first discovery.
Use `GRAPH_REPORT.md`, `graphify query`, `graphify explain`, `graphify path`, or
`graphify affected` to produce candidate files/symbols, then ask the external tool to
verify and fill gaps. If graph is absent or stale, use the normal external workflow.
...
- Include graph-derived candidate files/symbols when available and ask the tool to verify them
