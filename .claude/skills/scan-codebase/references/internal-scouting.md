...
Before spawning Explore agents, check for `graphify-out/graph.json` (Knowledge Graph is on by default). If present, load
`../../_shared/graphify-code-graph.md` from the skills root and run graph-first discovery:

- Read `graphify-out/GRAPH_REPORT.md`
- Use `graphify query` or `graphify explain` for the target area
- Use `graphify affected` for changed symbols/files
- Use `graphify path` when the task asks how two areas connect

Use graph results to seed agent scopes. Agents must verify graph leads with source reads or
targeted grep. If the graph is absent or stale, continue with the normal Explore workflow.
...
- If graph leads were provided, verify those first, then use Glob/Grep for missing areas
...
3. Mark graph-derived but unverified items as `graph-suggested`
4. Note any gaps/timeouts
