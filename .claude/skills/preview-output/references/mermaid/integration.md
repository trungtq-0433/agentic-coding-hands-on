...
Wiring Mermaid in through its JavaScript API, embedding it in HTML, and the quirks of specific platforms.

## HTML/Browser Integration

**The simplest CDN setup:**
```html
<!DOCTYPE html>
<html>
<head>
...
**mermaid.parse(text)**
Check the syntax without drawing anything.
```javascript
try {
...
**mermaid.run(config)**
Render every diagram on the page.
```javascript
await mermaid.run({
...
**Obsidian:**
Works in code blocks out of the box:
````markdown
```mermaid
graph TD
  A --> B
...
**PowerPoint/Word:**
Draw it in the mermaid.live editor, export, and drop the image in.
