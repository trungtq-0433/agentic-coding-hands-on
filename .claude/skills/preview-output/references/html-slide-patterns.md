...
**Step 4 — Plan visuals.** Plan visual interest for the deck using CSS gradients, SVG decorations, and any real/provided images. Target the **title slide** (16:9 background that sets the visual tone) and **one full-bleed slide** (immersive background for a key moment). Content slides with conceptual topics also benefit from a 1:1 illustration in the aside area. If a real image is provided, embed it as a base64 data URI; otherwise degrade to CSS gradients and SVG decorations.
...
**Visual priority order** for where to invest visual effort:

1. **Title slide** (always): background that sets the deck's visual tone. Use 16:9 aspect ratio. Match the topic and palette.
...
**Primary approach — CSS gradients and SVG decorations.** Use the `.slide__bg--gradient` pattern with bold `linear-gradient` or `radial-gradient` backgrounds, layered SVG shapes, and decorative accents pulled from the preset's CSS variables. This is the default and needs no external tooling.

**Real/provided images:** If the user provides real images, embed them as base64 data URIs:

```html
<!-- Embed a provided image as a slide background -->
<div class="slide__bg" style="background-image:url('data:image/png;base64,...')"></div>
```
