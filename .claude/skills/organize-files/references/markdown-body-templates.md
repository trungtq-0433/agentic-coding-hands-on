...
The skeleton each kind of markdown document is built on. SKILL.md Rule 4 gives the summary; the templates live here.

## Universal Rules

- One `# Title` (H1) per file, and it comes first
- Frontmatter (`---`) carries metadata whenever a tool or pipeline reads the file
- Sections flow context → content → next steps
- Structured data goes in tables, sequences in lists
- Trade grammar for concision
- Park the unresolved questions at the bottom
...
A plain doc needs no frontmatter — just sections that follow a logical order and are easy to scan.
...
Stick to the [Keep a Changelog](https://keepachangelog.com) conventions.
