...
Always, once per propose-improvements run, as part of Phase A. There is no separate "mechanical + verify" split — a single subagent is the sole classifier. If `plans/improvement-proposal/use-context.json` already exists and is non-empty, the orchestrator skips this step entirely (idempotency rule).
