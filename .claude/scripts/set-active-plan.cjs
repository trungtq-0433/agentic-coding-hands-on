 ...
 * for plan context within a session. Env vars ($TKM_ACTIVE_PLAN) are just
 * the initial snapshot from session start.
 */

const path = require('path');
const { updateSessionState } = require('../hooks/lib/tkm-config-utils.cjs');
...
if (!sessionId) {
  console.warn('Warning: TKM_SESSION_ID not set - session state will not persist');
